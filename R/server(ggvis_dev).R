...
shinyServer(function(input, output) {
  ...
  reactive(    
    subdf = subset(rws_dat, rws_dat$variable == input$substance &     # "ammonium" & #
                     rws_dat$locatie == input$location   &            #  "Terschelling 10 km uit de kust") #
    ...
    subdf$numtime <- as.numeric(subdf$datetime)/31622400,
    
    ## create plot 
    
    if(input$analysis == "trend") {   # in case trend is choice of analysis
      subdf %>% 
        ggvis(~numtime, ~waarde) %>% 
        layer_points() %>%
        # layer_model_predictions(model = "lm", se = T)
#         layer_model_predictions(model = "lm",
#                                 formula = waarde ~ numtime + I(cos(2*3.1416*numtime)) + I(sin(2*3.1416*numtime)),
#                                 se = T) %>%
        bind_shiny("tp", "tp_ui")
    }  ,
